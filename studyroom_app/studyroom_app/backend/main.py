...
class SignupReq(BaseModel):
    ...
    nickname: str = Field(min_length=1, max_length=32)
    pin: str = Field(min_length=4, max_length=32)
    signup_code: str | None = Field(default=None, max_length=64)
...
# App
# =========================================================
app = FastAPI(title="StudyRoom App", version="0.4.0")
...
def _require_signup_allowed(conn: sqlite3.Connection, provided_code: str | None) -> None:
    """
    Signup policy:
      - If users == 0 : allow (first-time setup, frictionless)
      - Else if STUDYROOM_SIGNUP_CODE is set: require match
      - Else: allow (open signup)
    """
    n = _users_count(conn)
    required = os.getenv("STUDYROOM_SIGNUP_CODE", "").strip()
    if n == 0:
        return
    if required:
        if (provided_code or "").strip() != required:
            raise HTTPException(status_code=401, detail="登録コードが違います（管理者に確認してください）")
        return
    return
...
def signup(req: SignupReq):
    """
    Self registration (for quick first-run).
    Recommended: set STUDYROOM_SIGNUP_CODE in .env for safety.
    """
    conn = db_connect()
    cur = conn.cursor()

    _require_signup_allowed(conn, req.signup_code)
    ...
    try:
        cur.execute("""
            ...
            VALUES (?, ?, ?, ?, ?)
        """, (req.student_no.strip(), req.name.strip(), req.nickname.strip(), pin_hash, iso(now_jst())))
        conn.commit()
